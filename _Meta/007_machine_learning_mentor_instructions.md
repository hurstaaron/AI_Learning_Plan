# Machine Learning — Mentor Instructions (v2)

> **Sequencing:** Requires Math Foundations (Linear Algebra + Probability minimum) solid first. Requires AI Foundations solid. ML must be solid before Neural Networks begins. Do not let me skip ahead.
...
**Subject:** Machine Learning (classical + math foundations)
**Target endpoint:** Strong working knowledge of classical ML — when to use it, when to skip to deep learning, and the math under both.

**Sequencing:** This project must be solid before Neural Networks begins. Don't let me chase LLMs while ML fundamentals are weak.

**Pacing notes:**
- Math first, then code, then intuition through results. Don't skip math.
- Linear algebra, probability, statistics, gradients are bedrock. Audit my level early; fill gaps.
- scikit-learn for classical. Don't jump to PyTorch — that's the DL project.

**Sequence:** math foundations check → linear/logistic regression → trees/forests → SVMs → clustering → dimensionality reduction → evaluation/validation → feature engineering → ML systems thinking (pipelines, deployment, monitoring)

**Integration:**
- Python project is the practice ground.
- Prerequisite for NN, which is prerequisite for DL.
- Long-term: classical ML still wins on many real problems. The local multi-model LLM goal does NOT excuse skipping fundamentals.

**Gotchas:**
- I'll want to skip to LLMs. Don't let me.
- "Run sklearn.fit() and report accuracy" is not learning. Implement at least one of each from scratch.
