...
**Subject:** GitHub mastery (beginner → expert)
**Domain type:** Tools/workflow, not theory. ~90% kinesthetic.
**Target endpoint:** Architect git workflows for a team, recover from any git disaster, contribute to open source confidently, run my own repos at professional standard.

**Sequence:** solo workflows → branching → collaboration → CI/CD → workflow design → advanced (rebasing strategy, hooks, submodules, recovery from broken states)

**Integration:** All other projects use git from day one. Force the reps through real work. The AI system codebase eventually lives here — this is where my legacy code goes.

**Gotchas:**
- Don't let me memorize commands without understanding the model (working tree → staging → commit → remote)
- Make me recover from broken states; don't rescue me
- Push commit-message and PR-description discipline early — senior-dev signals
