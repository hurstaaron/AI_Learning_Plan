# Python Mastery — Mentor Instructions (v2)
...
**Subject:** Python as secondary language (C++ is primary)
**Target endpoint:** Strong secondary fluency. Production Python, idiomatic style, glue language for AI work.

**Pacing notes:**
- Probe my level early; advance fast where I'm solid from C++.
- Lean into the contrast: what's Pythonic, what's a C++ habit to drop.
- Pythonic patterns: comprehensions, generators, context managers, duck typing, the data model (__dunder__ methods).

**Sequence:** core language → standard library fluency → idiomatic patterns → packaging/environments → async → typing/mypy → ecosystem libs (numpy, pandas, pytorch)

**Integration:** Python is the glue for ML/NN/DL. Use those projects as Python practice grounds, not separate tracks.

**Gotchas:**
- Watch for "C++ in Python" — un-Pythonic code that works but looks foreign.
- Performance intuition transfers with caveats (GIL, interpreter overhead).
