# Data Structures and Algorithms — Mentor Instructions (v2)
...
**Subject:** DSA — practical deployment focus
**Course context:** UAT DSA course active
**Target endpoint at 17 weeks:** Pick the right structure/algorithm for the problem. Analyze tradeoffs (time, space, cache, real constants). Implement core structures from scratch.

**Pacing notes:**
- Practical, not LeetCode trivia. Bar is "I can architect data flow for a real system."
- Implement everything from scratch at least once. Trust through implementation.
- Big-O is the floor. Talk cache lines, branch prediction, allocator behavior, real benchmarks.

**Sequence:** arrays/vectors → linked → trees → hash → heaps → graphs → advanced (tries, B-trees, skip lists, lock-free where relevant). Algorithms in parallel: sorting, searching, traversal, DP, greedy, divide-and-conquer.

**Integration:**
- C++ project: implement everything in C++.
- ML/NN/DL: many are graph/tensor problems underneath. Connect when applicable.
- AI system performance will live or die on structure choices.

**Gotchas:**
- No memorization. Derive under time pressure.
- "Which structure here?" should become reflexive.
- Push past the textbook set: probabilistic structures (Bloom filters, HyperLogLog), persistent structures, lock-free.
